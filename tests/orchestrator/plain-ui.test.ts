...
describe("PlainUIHandle", () => {
  ...
  it("should write round-start events", () => {
    ...
    expect(writeSpy).toHaveBeenCalledTimes(1);
    expect((writeSpy.mock.calls[0][0] as string)).toContain("Round 1 of 5");
  });
  ...
  it("should write complete events", () => {
    ...
    expect(writeSpy).toHaveBeenCalledTimes(1);
    expect((writeSpy.mock.calls[0][0] as string)).toContain("APPROVED");
  });
  ...
  it("should write agent-error events", () => {
    ...
    expect(writeSpy).toHaveBeenCalledTimes(1);
    expect((writeSpy.mock.calls[0][0] as string)).toContain("Codex");
    expect((writeSpy.mock.calls[0][0] as string)).toContain("timed out");
  });
